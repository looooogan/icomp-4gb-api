...
name: '后台管理员',
job: 'administrator',
picture: 'app/img/user/admin.jpg'
};
...
App.config(['$stateProvider', '$locationProvider', '$urlRouterProvider', 'RouteHelpersProvider', 'RestangularProvider',
function ($stateProvider, $locationProvider, $urlRouterProvider, helper, RestangularProvider) {
'use strict';
...
.state('app.${(entity.name)!}', {
url: '/${(entity.name)!}',
title: '${(entity.comment)!}管理',
templateUrl: helper.basepath('${(entity.tablePackageInfo['model'])!}/${(entity.name)!}.html'),
resolve: helper.resolveFor('localytics.directives','ngGrid')
})
.state('app.${(entity.name)!}InsAndUpd', {
url: '/${(entity.name)!}/insAndUpd/:param',
title: '${(entity.comment)!}信息维护',
templateUrl: helper.basepath('${(entity.tablePackageInfo['model'])!}/${(entity.name)!}_InsAndUpd.html'),
resolve: helper.resolveFor('filestyle', 'taginput'),
params: {
'entity': {}
}
})
...
/**=========================================================
* Module: ${(entity.name)!}
* ${(entity.name)!} service controller
=========================================================*/
App.service("${(entity.name)!}Service", function (Restangular) {
return {
list: function (entity) {
return Restangular.all('${(entity.name)!}/list').post(entity);
},
delete: function (entity) {
return Restangular.all('${(entity.name)!}/del').post(entity);
},
upd: function (entity) {
return Restangular.all('${(entity.name)!}/upd').post(entity);
},
add: function(entity){
return Restangular.all('${(entity.name)!}/add').post(entity);
}
}
});


App.controller('${(entity.name)!}Controller', ['$scope', '$state', '${(entity.name)!}Service','$timeout',
function ($scope,$state, ${(entity.name)!}Service,$timeout) {
'use strict';
//添加 修改 删除
$scope.entity = {};
//查询表单
...
$scope.add = function(){
$state.go('app.${(entity.name)!}InsAndUpd');
}

$scope.update = function () {
if($scope.gridOptions.selectedItems.length!=1){
$scope.pop('warning','系统提示','请选择一个修改项');
return;
}
$scope.entity = $scope.gridOptions.selectedItems[0];
$state.go('app.${(entity.name)!}InsAndUpd', {'entity': $scope.entity});
}

$scope.delete = function () {
if($scope.gridOptions.selectedItems.length<=0){
$scope.pop('Error','系统提示','需要选择删除项');
return;
}
$scope.entity = $scope.gridOptions.selectedItems[0];
$scope.isShowSpinner =true;
${(entity.name)!}Service.delete($scope.entity).then(function(data,status,headers){
$scope.isShowSpinner =false;
$scope.pop('success','系统提示','删除成功');
$scope.getPagedDataAsync($scope.pagingOptions.pageSize, $scope.pagingOptions.currentPage);
});
...
$scope.pagingOptions = {
pageSizes: [10],  // page size options
...
<#list (entity.columns)? keys  as  key>
    ...
    <#if !(column.exportkey) && !(column.importKey) && !(column.manyToManyKey)&&!(column.identifyKey)>
    {field:'${(column.name)!}', displayName:'${(column.comment)!}'},
    </#if>
</#list>
<#list (entity.columns)? keys  as  key>
    <#assign column=entity.columns[key]>
    <#if (column.importKey)>
        ...
        <#list (importTableColumns)? keys as exportKey>
    {field:'${(column.importTable.name)!}.${(importTableColumns[exportKey].name)!}', displayName:'${(importTableColumns[exportKey].comment)!}'},
        </#list>
    </#if>
</#list>
<#list (entity.columns)? keys  as  key>
...
//{field:'expandInfo.sex', displayName:'性别',cellTemplate:'<div class="ngCellText ng-scope" ng-switch="row.getProperty(col.field)"><span ng-switch-when="1" text>男</span><span ng-switch-when="2">女</span></div>'},
]
};
...
${(entity.name)!}Service.list($scope.searchEntity).then(function(largeLoad,status,headers){
...
App.controller('${(entity.name)!}InsAndUpdController', ['$scope', '$state','${(entity.name)!}Service','$stateParams',
function ($scope, $state,${(entity.name)!}Service,$stateParams) {
'use strict';

$scope.entity = $stateParams.entity;
$scope.toList = function () {
$state.go('app.${(entity.name)!}');
}
...
$scope.isShowSpinner = true;
if($scope.entity.id){
$scope.submitFunction = ${(entity.name)!}Service.upd($scope.entity);
}else{
$scope.submitFunction = ${(entity.name)!}Service.add($scope.entity);
}
$scope.submitFunction.then(function(data,status,headers){
$scope.isShowSpinner = false;
$scope.pop('success','系统提示','操作成功');
},function(data,status,headers){
$scope.isShowSpinner = false;
$scope.pop('error','系统提示','操作失败');
